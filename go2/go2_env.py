...
@configclass
class Go2MPCCfg(InteractiveSceneCfg):
    # ground plane
    ground = AssetBaseCfg(
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        base_lin_vel = ObsTerm(
            func=mdp.base_lin_vel, params={"asset_cfg": SceneEntityCfg(name="unitree_go2")}
        )
        base_ang_vel = ObsTerm(
            func=mdp.base_ang_vel, params={"asset_cfg": SceneEntityCfg(name="unitree_go2")}
        )
        ...
        joint_pos = ObsTerm(
            func=mdp.joint_pos_rel, params={"asset_cfg": SceneEntityCfg(name="unitree_go2")}
        )
        joint_vel = ObsTerm(
            func=mdp.joint_vel_rel, params={"asset_cfg": SceneEntityCfg(name="unitree_go2")}
        )
...
@configclass
class Go2MPCEnvCfg(ManagerBasedRLEnvCfg):
...
def camera_follow(env):
    if env.unwrapped.scene.num_envs == 1:
        ...
        robot_orientation = (
            env.unwrapped.scene["unitree_go2"].data.root_state_w[0, 3:7].cpu().numpy()
        )
        ...
        set_camera_view(
            yaw_rotation.dot(np.asarray([-4.0, 0.0, 5.0])) + robot_position, robot_position
        )
